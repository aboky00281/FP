...
<body>
    <div class="container">
        ...
        <div id="results_page" class="pt-8">
            <div id="results" class="results-section">
                ...
                <p>
                    <span>อัตราการไหลรวมที่ต้องการ (ขนาดปั๊มดับเพลิง):</span> <span id="total_flow" class="text-red-600 font-normal">0.00</span> GPM
                </p>
            ...
            <div id="graph_summary" class="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                ...
                <div id="graph_summary_content" class="hidden">
                    <ul class="list-disc ml-5 text-gray-700 text-sm">
                        <!-- Removed the "ช่วงแรงดันการทำงานที่แนะนำ (สีส้มอ่อน)" explanation -->
            ...
            <table id="calculation_summary_table" class="w-full border-collapse border border-gray-300 rounded-lg overflow-hidden">
                ...
                <tbody>
                    <tr>
                        ...
                        <td class="py-2 px-4 border-b border-gray-300"><span id="summary_sprinkler_flow">0.00</span> GPM</td>
                    </tr>
                    <tr>
                        <td class="py-2 px-4 border-b border-gray-300">อัตราการไหลสำหรับท่อยืน (Hose Stream Allowance)</td>
                        <td class="py-2 px-4 border-b border-gray-300"><span id="summary_hose_allowance">0.00</span> GPM</td>
                    </tr>
                    <tr>
                        <td class="py-2 px-4 border-b border-gray-300 font-bold">อัตราการไหลรวมที่ต้องการ (ขนาดปั๊มดับเพลิง)</td>
                        <td class="py-2 px-4 border-b border-gray-300 text-red-600 font-normal"><span id="summary_total_flow">0.00</span> GPM</td>
                    </tr>
                    <tr>
                        <td class="py-2 px-4 border-b border-gray-300 font-bold">แรงดันที่จุดต้องการ</td>
                        <td class="py-2 px-4 border-b border-gray-300 font-normal"><span id="summary_demand_head">0.00</span> PSI</td>
                    </tr>
                    <tr>
                        <td class="py-2 px-4 border-b border-gray-300">ขนาดถังเก็บน้ำที่ต้องการ</td>
                        <td class="py-2 px-4 border-b border-gray-300"><span id="summary_tank_volume">0.00</span> ลบ.ม.</td>
                    </tr>
                    <tr>
                        <td class="py-2 px-4 border-b border-gray-300">เวลากักเก็บน้ำที่คำนวณ</td>
                        <td class="py-2 px-4 border-b border-gray-300"><span id="summary_actual_duration">0</span> นาที</td>
                    </tr>
                    <tr>
                        <td class="py-2 px-4 border-b border-gray-300 font-bold">อัตราไหลของปั๊มที่ควรเลือก (โดยประมาณ)</td>
                        <td class="py-2 px-4 border-b border-gray-300 font-bold text-green-600"><span id="summary_recommended_pump_flow">0.00</span> GPM</td>
                    </tr>
                    <tr>
                        <td class="py-2 px-4 border-b border-gray-300 font-bold">แรงดันโดยประมาณ</td>
                        <td class="py-2 px-4 border-b border-gray-300 font-bold text-green-600"><span id="summary_estimated_pressure">0.00</span> PSI</td>
                    </tr>
                </tbody>
            </table>
        </div>
    ...
    <script>
        ...
        function toggleGraphSummary() {
            document.getElementById('graph_summary_content').classList.toggle('hidden');
        }
        ...
        function updatePumpCurveChart(pumpRatedFlow, totalRequiredFlow, systemDemandHeadInput) {
            ...
            // The user requested to remove the +65 PSI calculation from estimatedPressureOnGraph
            ...
            // Ensure Y-axis covers systemDemandHeadInput and dynamically calculated shutOffHead
            ...
            const chartOptions = {
                    ...
                    x: {
                        ...
                        ticks: {
                            ...
                            callback: function(value) {
                                return value.toFixed(0); // Display as whole number
                            }
                        }
                    },
                    y: {
                ...
                plugins: {
                    ...
                    datalabels: { // Global datalabels configuration (can be overridden per dataset)
                        display: false // Hide by default
                    }
                }
            };

            // Removed the "ช่วงแรงดันการทำงานที่แนะนำ" (Recommended Operating Pressure Range) highlight dataset.


            // 1. Performance Curve (Teal line)
            ...
            // 2. Shut-off Head (Max. churn) line (Dark red dashed)
            if (!isNaN(ratedHeadForCurve) && ratedHeadForCurve > 0) {
                chartData.datasets.push({
                    ...
                    fill: false,
                    borderColor: 'rgb(128, 0, 0)', // Changed to maroon (เลือดหมู)
                    borderDash: [6, 6],
                    pointRadius: 0,
                    hoverBorderWidth: 0,
                    tension: 0,
                    order: 3,
                    datalabels: {
                        display: function(context) { // Display only for the first point (left side)
                            return context.dataIndex === 0;
                        },
                        align: 'top', 
                        anchor: 'center', 
                        offset: -5, 
                        color: 'rgb(128, 0, 0)', // Changed to maroon (เลือดหมู)
                        font: {
                            weight: 'bold'
                        },
            ...
            // 3. Rated Point (Yellow circle) and its associated dashed lines
            if (!isNaN(pumpRatedFlow) && pumpRatedFlow > 0 && !isNaN(ratedHeadForCurve) && ratedHeadForCurve > 0) {
                ...
                chartData.datasets.push({
                    ...
                    datalabels: {
                        ...
                        align: 'end',
                        anchor: 'end',
                        color: 'blue', // Label color for Rated Point
                        font: {
                            weight: 'bold'
                        },
                        formatter: function(value, context) {
                ...
                // Vertical dashed line for Rated GPM (blue and dotted)
                chartData.datasets.push({
                    type: 'line',
                    label: 'เส้นแสดง Rated GPM (สีน้ำเงิน)', // Updated label to reflect color
                    data: [
                        { x: pumpRatedFlow, y: 0 },
                        { x: pumpRatedFlow, y: ratedHeadForCurve }
                    ],
                    fill: false,
                    borderColor: 'rgba(0, 0, 255, 0.7)', // Changed to blue
                    borderDash: [5, 5],
                    pointRadius: 0,
                    hoverBorderWidth: 0,
                    tension: 0,
                ...
                // Horizontal dashed line from Rated Point to Y-axis (blue and dotted)
                chartData.datasets.push({
                    type: 'line',
                    label: 'เส้นแสดงแรงดันกำหนดจากจุด Rated Point (สีน้ำเงิน)', // Updated label to reflect color
                    data: [
            ...
            // 4. 150% Flow, 65% Head point (Purple rectRot) and its associated purple dashed lines
            if (pumpRatedFlow > 0 && ratedHeadForCurve > 0) {
                chartData.datasets.push({
                    type: 'scatter',
                    ...
                    datalabels: {
                        ...
                        align: 'end',
                        anchor: 'end',
                        color: 'rgba(153, 102, 255, 1)', // Changed to match line/point color (purple)
                        font: {
                            weight: 'bold'
                        },
                        formatter: function(value, context) {
            ...
            // 5. Add Total Required Flow as a red star point
            if (totalRequiredFlow > 0) {
                chartData.datasets.push({
                    type: 'scatter',
                    label: `อัตราไหลรวมที่ต้องการ (Demand Point - รูปดาวแดง)`,
                    data: [{ x: totalRequiredFlow, y: systemDemandHeadInput }], // Use systemDemandHeadInput for demand point Y-coordinate
                    ...
                    datalabels: {
                        display: true,
                        align: 'bottom', // Changed to bottom
                        anchor: 'center', // Changed to center
                        offset: 10, // Add a small offset to move it further below
                        color: 'red',
                        font: {
                            weight: 'bold'
                        },
                        formatter: function(value, context) {
                            return `Demand: ${value.x.toFixed(0)} GPM`; // Display only GPM
                        }
                    }
                });
            }
        ...
        function calculate() {
            ...
            // Re-introduced warningMessage variable and element
            let warningMessage = '';
            ...
            if (isNaN(designAreaSqM) || designAreaSqM <= 0) {
                ...
                updatePumpCurveChart(0, 0, 0); // Also reset chart on error
                return;
            } else {
                warningMessageElement.classList.add('hidden'); // Hide warning if input is valid
            }
            ...
            // The following block is removed as requested by the user
            /*
            ...
            */
            ...
            document.getElementById('summary_sprinkler_flow').textContent = sprinklerFlowGPM.toFixed(2);
            document.getElementById('summary_hose_allowance').textContent = hoseAllowanceGPM.toFixed(2);
            document.getElementById('summary_total_flow').textContent = totalFlowGPM.toFixed(2);
            document.getElementById('summary_tank_volume').textContent = requiredTankVolumeCubicMeters.toFixed(2);
            document.getElementById('summary_actual_duration').textContent = actualDurationMinutes;
            
            // Set the recommended pump flow from the pump_rated_flow input
            document.getElementById('summary_recommended_pump_flow').textContent = (!isNaN(pumpRatedFlow) && pumpRatedFlow > 0) ? pumpRatedFlow.toFixed(2) : '0.00';
            ...
            if (!isNaN(pumpRatedFlow) && pumpRatedFlow > 0) {
                ...
                // The user requested to remove the +65 PSI calculation from estimatedPressureWithAddition
                ...
                demandHeadDisplayElement.textContent = calculatedRequiredHead.toFixed(0); // Changed to toFixed(0)
                demandHeadDisplayContainer.classList.remove('hidden');
                demandHeadCalculationElement.textContent = demandHeadCalculationText;
                document.getElementById('summary_demand_head').textContent = calculatedRequiredHead.toFixed(0); // Changed to toFixed(0)
                document.getElementById('summary_estimated_pressure').textContent = estimatedPressureWithAddition.toFixed(0); // Changed to toFixed(0)
                ...
                document.getElementById('summary_demand_head').textContent = '0.00';
                document.getElementById('summary_estimated_pressure').textContent = '0.00'; 
        ...
        function clearData() {
            ...
            // Clear summary table
            document.getElementById('summary_sprinkler_flow').textContent = '0.00';
            document.getElementById('summary_hose_allowance').textContent = '0.00';
            document.getElementById('summary_total_flow').textContent = '0.00';
            document.getElementById('summary_demand_head').textContent = '0.00';
            document.getElementById('summary_tank_volume').textContent = '0.00';
            document.getElementById('summary_actual_duration').textContent = '0';
            document.getElementById('summary_recommended_pump_flow').textContent = '0.00';
            document.getElementById('summary_estimated_pressure').textContent = '0.00'; 
            ...
            // Re-enabled warning message clearing
